...
function HomePage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY <= heroBottom) {
        // Still in hero area, hide sticky search
        setShowStickySearch(false);
      } else if (currentScrollY < lastScrollY.current) {
        // Scrolling up and past hero - show sticky search
        setShowStickySearch(true);
      } else {
        // Scrolling down past hero - hide sticky search
        setShowStickySearch(false);
      }
  ...
  const filteredRestaurants = restaurants
    .filter((restaurant) => {
      ...
      // Category filter
      if (selectedCategory && restaurant.cuisine !== selectedCategory) {
        return false;
      }
      ...
      // Open now filter
      if (filters.openNow && !isOpenNow(restaurant)) {
        return false;
      }
      ...
      if (filters.outdoorSeating && !restaurant.outdoorSeating) {
        return false;
      }

      // Family friendly filter
      if (filters.familyFriendly && !restaurant.familyFriendly) {
        return false;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <section
        ref={heroRef}
        className="relative bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 py-20 px-4 pb-28"
      >
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          >
            Discover and book the best restaurants in your city. From cozy
            dinners to grand celebrations, find the perfect spot for every
            occasion.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
                ...
                <div className="mt-4 p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-100 flex flex-wrap gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={filters.reservationToday}
                        onChange={(e) =>
                          setFilters((prev) => ({
                            ...prev,
                            reservationToday: e.target.checked,
                          }))
                        }
                        className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                      />
                      <span className="text-sm text-gray-700">
                        Reservation Available Today
                      </span>
                    </label>
