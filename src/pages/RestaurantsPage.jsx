...
export default function RestaurantsPage() {
  ...
  const filteredRestaurants = restaurants
    .filter((restaurant) => {
      ...
      if (selectedCategory && restaurant.cuisine !== selectedCategory) return false;
          ...
          <div className="flex flex-wrap gap-3 py-2">
            {(showAllCategories ? categories : categories.slice(0, VISIBLE_CATEGORIES)).map((category) => (
              ...
              >
                ...
                <span className={`text-xs font-medium ${selectedCategory === category.id ? 'text-indigo-700' : 'text-gray-600'}`}>
                  {category.name}
                </span>
              </motion.button>
            ))}
